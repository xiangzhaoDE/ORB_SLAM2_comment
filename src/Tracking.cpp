    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    // @XZ: FileStorage类, 浮点数据或者XML/YAML文件的读写; imread(), imwrite()只能读写整数数据, 且需要以图像格式
    ...
    // K = |0   fy  cy|
    //     |0   0   1 |
    // @XZ: focal length, camera principal point
    cv::Mat K = cv::Mat::eye(3,3,CV_32F);
    // @XZ: 矩阵初始化, Returns an identity matrix(单位矩阵) of the specified size and type
    K.at<float>(0,0) = fx;
    K.at<float>(1,1) = fy;
    K.at<float>(0,2) = cx;
    K.at<float>(1,2) = cy;
    K.copyTo(mK);   // @XZ: 深拷贝, mK需事先声明
    ...
    // 双目摄像头baseline * fx 50
    mbf = fSettings["Camera.bf"];   // @XZ: bf: baseline * fx
